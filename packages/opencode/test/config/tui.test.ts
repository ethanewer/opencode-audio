...
test("loads shell_view from tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "tui.json"), JSON.stringify({ shell_view: "expanded" }, null, 2))
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const config = await TuiConfig.get()
      expect(config.shell_view).toBe("expanded")
    },
  })
})

test("shell_view defaults to undefined when not set", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const config = await TuiConfig.get()
      expect(config.shell_view).toBeUndefined()
    },
  })
})

test("migrates shell_view from legacy opencode.json tui key", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "opencode.json"),
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const config = await TuiConfig.get()
      expect(config.shell_view).toBe("compressed")
      const text = await Filesystem.readText(path.join(tmp.path, "tui.json"))
      expect(JSON.parse(text)).toMatchObject({ shell_view: "compressed" })
    },
  })
})

test("rejects invalid shell_view values in tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "tui.json"), JSON.stringify({ shell_view: "invalid" }, null, 2))
    },
  })
